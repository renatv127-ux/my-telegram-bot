...
async def start_cmd(message: types.Message):
    ...
    await message.answer(f"Привет, {message.from_user.first_name}! Чтобы получить драм кит, подпишись на канал и жми кнопку!" + CONTACT_INFO, reply_markup=kb)
...
async def process_download(callback: types.CallbackQuery):
    ...
    # 2. Проверка файла (загрузил ли его админ)
    file_id = get_setting("file_id")
    if not file_id:
        await callback.answer("❌ Файл еще не добавлен админом. Жди анонса!", show_alert=True)
        return
    ...
    # --- 4. ЖЕЛЕЗНОЕ ОБНОВЛЕНИЕ СТАТУСА ---
    # Сначала проверяем, первый ли раз человек качает (для статистики)
    if not u_data or u_data[1] == 0:
        cursor.execute("UPDATE settings SET value = CAST(value AS INTEGER) + 1 WHERE key='downloads'")

    # Принудительно вставляем или обновляем статус "received_file = 1"
    # Это решает проблему с /grade
...
async def cmd_grade(message: types.Message, state: FSMContext):
    ...
    # Теперь проверка будет работать на 100%
    if not row or row[0] == 0:
        await message.answer("❌ Бот не видит, что ты скачивал файл. Нажми 'Скачать драм кит' в /start!" + CONTACT_INFO)
        return
    ...
    if existing:
        ...
        text = f"🔄 Твой отзыв ({existing[0]}/5). Можно обновить оценку:"
    else:
        text = "⭐ Оцени драм кит от 1 до 5:"
...
async def admin_file_req(message: types.Message, state: FSMContext):
    if message.from_user.id == ADMIN_ID:
        await message.answer("📁 Отправь файл драм-кита (как ДОКУМЕНТ):")
...
async def admin_file_save(message: types.Message, state: FSMContext):
    if message.from_user.id == ADMIN_ID:
        ...
        conn.commit()
        await message.answer("✅ Файл сохранен и доступен всем!")
        await state.clear()
