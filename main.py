...
async def cmd_grade(message: types.Message, state: FSMContext):
    ...
    if existing:
        text = f"🔄 У тебя уже есть отзыв ({existing[0]}/5). Давай обновим его!\nВыбери новую оценку:"
...
async def process_comment(message: types.Message, state: FSMContext):
    ...
    conn.commit()
    await message.answer("✅ Твой отзыв сохранен!" + CONTACT_INFO)
    await state.clear()
...
async def cmd_view_reviews(message: types.Message):
    ...
    for r in rows:
        res += f"👤 @{r[0]} | {r[1]}/5\n📝 {r[2]}\n📅 {r[3]}\n\n"
    await message.answer(res if rows else "Отзывов пока нет." + CONTACT_INFO)
...
async def start_cmd(message: types.Message):
    ...
    await message.answer("Привет! Чтобы скачать драм кит, подпишись на наш канал!", reply_markup=kb)
...
async def process_sub(callback: types.CallbackQuery):
    if await is_subscribed(callback.from_user.id):
        ...
        await callback.message.answer_document(file_res[0], caption=f"🥁 Файл готов! Скачиваний: {total}\n\nОставь отзыв: /grade" + CONTACT_INFO)
        await callback.answer()
    else:
        await callback.message.answer("❌ Подписка не найдена!" + CONTACT_INFO)
        await callback.answer()
...
async def admin_menu(message: types.Message):
    ...
    msg = (
        ...
        "/sms [текст] — Рассылка\n"
        "/delete_review [ID] — Удалить отзыв (ID юзера)\n\n"
        "👤 **Команды юзера:**\n"
        "/start, /grade, /review"
    )
...
async def del_rev(message: types.Message):
    if message.from_user.id == ADMIN_ID:
        ...
        except: await message.answer("Пример: /delete_review 1753037099")

@dp.message(Command("FileDK"))
async def ask_file(message: types.Message):
    if message.from_user.id == ADMIN_ID:
        await message.answer("Скинь файл драм-кита.")
