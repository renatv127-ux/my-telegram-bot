...
def db_init():
    ...
    try: cursor.execute("ALTER TABLE users ADD COLUMN last_msg_id INTEGER")
    except: pass
...
async def admin_panel(message: types.Message):
    ...
    f_val = get_setting("file_id")
    f_status = "✅ ОК" if f_val else "❌ НЕТ ФАЙЛА"
...
# Обработчик ошибки (если прислали не файл)
@dp.message(AdminStates.waiting_for_file)
async def admin_file_wrong(message: types.Message):
    if message.from_user.id != ADMIN_ID: return
    await message.answer("❌ Это не файл! Пожалуйста, отправь именно ФАЙЛ (документ).")
...
async def admin_del_review(message: types.Message):
    ...
    try:
        ...
        if cursor.fetchone():
            ...
            conn.commit()
            await message.answer(f"✅ Отзыв пользователя {rid} удален. Статистика скачиваний сохранена.")
        else:
            await message.answer(f"❌ Отзыва от ID {rid} не существует.")
    except: await message.answer("❌ Ошибка. Введи ID числом.")
