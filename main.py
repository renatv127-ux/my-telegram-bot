...
TEXTS = {
    'ru': {
        ...
        'ready': " drumming <b>Ваш файл готов!</b>\n📈 Скачало человек: {}\n⭐️ Рейтинг: {}/5\n\n/grade — оставить отзыв\n/review — все отзывы\n\nесли будут вопросы или проблемы, пиши в лс @TwixerArtist",
    ...
    'en': {
        ...
        'ready': " drumming <b>Your file is ready!</b>\n📈 Downloaded: {}\n⭐️ Rating: {}/5\n\n/grade — leave review\n/review — all reviews\n\nquestions? write to @TwixerArtist",
...
def db_init():
    cursor.execute('''CREATE TABLE IF NOT EXISTS users 
                      (user_id INTEGER PRIMARY KEY, username TEXT, full_name TEXT, 
                       ...
                       last_download_time REAL DEFAULT 0, join_date TEXT, lang TEXT DEFAULT 'ru')''')
    ...
    # Пытаемся добавить колонку lang, если её нет (для старых баз)
    try:
...
async def check_bot_status(message: types.Message):
    if get_setting("bot_status") == "off" and message.from_user.id != ADMIN_ID:
        ...
        await message.answer(TEXTS[lang]['bot_off'])
        return False
    return True
...
async def show_reviews_page(message_or_call, page: int):
    ...
    if not rows:
        ...
        text = TEXTS[lang]['rev_header'].format(page + 1, max(1, total_pages))
        for r in rows:
            text += TEXTS[lang]['rev_line'].format(r[0], '⭐'*r[1], r[2], r[3])
        nav_btns = []
        if page > 0: nav_btns.append(InlineKeyboardButton(text=TEXTS[lang]['back'], callback_data=f"rvp_{page-1}"))
        if page < total_pages - 1: nav_btns.append(InlineKeyboardButton(text=TEXTS[lang]['next'], callback_data=f"rvp_{page+1}"))
...
async def cmd_start(message: types.Message, state: FSMContext):
    ...
    # Выбор языка при первом старте или если сброшен
...
async def process_lang_selection(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.delete() # Удаляем выбор языка
    await state.clear()
    
    if not await check_bot_status(callback.message): return
...
async def process_dl(callback: types.CallbackQuery):
    ...
    if get_setting("bot_status") == "off" and user_id != ADMIN_ID:
        await callback.answer(TEXTS[lang]['bot_off'], show_alert=True); return
...
async def sk_comment(call: types.CallbackQuery, state: FSMContext):
    ...
    conn.commit(); await call.message.edit_text(TEXTS[lang]['saved']); await state.clear()
...
async def save_rev(message: types.Message, state: FSMContext):
    ...
    conn.commit(); await message.answer(TEXTS[lang]['saved']); await state.clear()
...
async def full_stats_cmd(message: types.Message):
    ...
    cursor.execute("SELECT username, lang FROM users ORDER BY rowid DESC LIMIT 10")
