...
def get_setting(key):
    ...
    res = cursor.fetchone()
    if res and res[0] and len(str(res[0]).strip()) > 0:
        return res[0]
    return None
...
async def admin_panel(message: types.Message):
    ...
    # Проверка файла напрямую из базы для админки
    cursor.execute("SELECT value FROM settings WHERE key='file_id'")
    f_val = cursor.fetchone()
    f_status = "✅ ОК" if (f_val and f_val[0] != "") else "❌ НЕТ ФАЙЛА"
...
async def admin_file_save(message: types.Message, state: FSMContext):
    ...
    cursor.execute("UPDATE settings SET value=? WHERE key='file_id'", (fid,))
    conn.commit()
...
async def admin_file_wrong(message: types.Message):
    if message.from_user.id != ADMIN_ID: return
    await message.answer("❌ Это не документ! Отправь именно файл.")
...
async def admin_del_review(message: types.Message):
    ...
    try:
        ...
        if cursor.fetchone():
            ...
            await message.answer(f"❌ Отзыва от {rid} нет.")
