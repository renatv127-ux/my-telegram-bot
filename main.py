    ...
    if existing:
        ...
        try:
            ...
            if datetime.now(MSK) - review_dt > timedelta(hours=2):
                await message.answer("❌ Изменить отзыв можно только в течение 2-х часов после его публикации!" + CONTACT_INFO)
                return
        except:
            pass
...
async def cmd_view_reviews(message: types.Message):
    ...
    for r in rows:
        ...
        # Если пишет админ - добавляем ID в текст
...
async def process_sub(callback: types.CallbackQuery):
    ...
    if await is_subscribed(user_id):
        ...
        if not file_res:
            await callback.answer("❌ Ошибка: Файл еще не загружен админом.", show_alert=True)
            return
            ...
            if user_data[1]:
                try:
                    ...
                    if now_dt - last_dt < timedelta(minutes=15):
                        ...
                        await callback.answer(f"❌ Повторная загрузка доступна раз в 15 минут!\nПодожди еще: {mins}м {secs}с", show_alert=True)
                        return
                except: pass

        if not user_data:
            cursor.execute("INSERT INTO users (user_id, username, full_name, received_file, last_download_time, date_received) VALUES (?, ?, ?, 1, ?, ?)",
        ...
        avg, count = get_average_rating()
        caption_text = (f"🥁 <b>Файл готов!</b>\n📈 Всего скачиваний: {total}\n⭐ Рейтинг: {avg}/5 (отзывов: {count})\n\n/grade — написать отзыв\n/review — отзывы{CONTACT_INFO}")
...
async def ask_file(message: types.Message):
    if message.from_user.id == ADMIN_ID:
        await message.answer("Пришли файл драм-кита (документом).")
...
async def save_file(message: types.Message):
    if message.from_user.id == ADMIN_ID:
        ...
        conn.commit()
        await message.answer("✅ Файл сохранен! Теперь он доступен всем пользователям.")
