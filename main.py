...
async def broadcast_sms(message: types.Message):
    ...
    for user in users:
        ...
        except TelegramForbiddenError: # Исправлено здесь
            continue
        except Exception:
            continue
...
async def admin_panel(message: types.Message):
    await message.answer("Команды: /on, /off, /FileDK, /Stata, /sms текст")
...
async def get_stata(message: types.Message):
    ...
    if not rows:
        await message.answer("Нет скачиваний.")
        return
    report = "Статистика:\n"
    for row in rows:
        report += f"{row[1] if row[1] else row[2]} | {row[3]}\n"
