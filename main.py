...
async def start_cmd(message: types.Message):
    ...
    await message.answer(f"Привет! Подпишись на канал и нажми кнопку, чтобы получить файл!" + CONTACT_INFO, reply_markup=kb)
...
async def process_download(callback: types.CallbackQuery):
    ...
    # --- ЖЕЛЕЗНОЕ ИСПРАВЛЕНИЕ ДЛЯ ОТЗЫВОВ ---
    # Мы используем INSERT OR REPLACE, чтобы запись ТОЧНО была в базе с received_file = 1
...
async def cmd_grade(message: types.Message, state: FSMContext):
    ...
    # Прямая проверка: есть ли пометка о получении
    cursor.execute("SELECT received_file FROM users WHERE user_id=?", (user_id,))
    row = cursor.fetchone()
    
    if not row or row[0] == 0:
        await message.answer("❌ Бот не видит, что ты получил файл. Сначала нажми 'Скачать драм кит' в /start!" + CONTACT_INFO)
        return
