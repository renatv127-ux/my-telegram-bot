...
async def process_download(callback: types.CallbackQuery):
    ...
    # --- ИСПРАВЛЕННАЯ ПРОВЕРКА КУЛДАУНА ---
    ...
    if u_data and u_data[0]:
        try:
            last_time = float(u_data[0])
            time_passed = current_time - last_time
            if time_passed < 300:
                left = int(300 - time_passed)
                await callback.answer(f"⏳ Подождите {left // 60}м {left % 60}с.", show_alert=True)
                return
        except (ValueError, TypeError):
            # Если в базе старый формат времени (текст), просто сбрасываем его
    ...
    async with download_queue:
        ...
        cursor.execute("""UPDATE users SET 
                          ...
                          WHERE user_id=?""", (current_time, date_str, user_id))
        conn.commit()
        ...
        try:
            ...
            await callback.message.answer("❌ Ошибка отправки. Напишите @TwixerArtist")
...
async def admin_panel(message: types.Message):
    ...
    await message.answer(f"🛠 <b>Админ-панель</b>\n\nФайл: {f_status}\nБот: {get_setting('bot_status')}\n\n"
                         ...
                         f"/on | /off — Состояние\n"
                         f"/delete_review [ID] — Удалить один отзыв\n"
                         f"/clear_reviews — Удалить ВСЕ отзывы\n"
                         f"/clear_stata — Обнулить ВСЕ скачивания")

@dp.message(Command("clear_reviews"), F.from_user.id == ADMIN_ID)
async def clear_all_reviews(message: types.Message):
    cursor.execute("DELETE FROM reviews")
    conn.commit()
    await message.answer("✅ Все отзывы полностью удалены.")

@dp.message(Command("clear_stata"), F.from_user.id == ADMIN_ID)
async def clear_all_stata(message: types.Message):
    # Обнуляем статус скачивания и время у всех пользователей
    cursor.execute("UPDATE users SET received_file=0, last_download_time=0, date_received=NULL")
    conn.commit()
    await message.answer("✅ Статистика обнулена. Теперь все могут скачать файл заново.")
