...
async def check_sub_callback(callback: types.CallbackQuery):
    ...
    if subscribed:
        ...
        # Проверяем уникальность скачивания
...
async def get_stata(message: types.Message):
    ...
    report = "📊 **Статистика скачиваний:**\n\n"
    
    for row in rows:
        uid, uname, fname, date = row
        
        # Проверяем подписку в реальном времени
        is_sub = await is_subscribed(uid)
        sub_text = "✅ подписан" if is_sub else "❌ отписался"
        
        # Формируем имя
        user_name = f"@{uname}" if uname else f"[{fname}](tg://user?id={uid})"
        
        report += f"👤 {user_name}\n┣ Статус: {sub_text}\n┗ Время: {date}\n\n"
    
    # ТГ ограничивает длину сообщения, если людей много - режем
    if len(report) > 4000:
