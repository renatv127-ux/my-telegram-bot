...
# --- 1. СОСТОЯНИЯ (ReviewStates должен быть в начале) ---
...
TEXTS = {
    'ru': {
        ...
        'no_lang_err': "Сначала выберите язык / Please choose language first:"
    },
    'en': {
        ...
        'lang_confirm': "✅ Language set: English!",
        'lang_ask': "Choose language / Выберите язык:",
        'no_lang_err': "Please choose language first / Сначала выберите язык:"
    }
}
...
async def cmd_start(message: types.Message, state: FSMContext):
    ...
    # Если языка еще нет - показываем выбор
    if lang is None:
...
async def set_language(callback: types.CallbackQuery, state: FSMContext):
    ...
    # Сохраняем в базу (с коммитом, чтобы не пропадало)
...
async def start_download(callback: types.CallbackQuery):
    ...
    async with download_queue:
        ...
        cursor.execute("SELECT COUNT(*) FROM users WHERE received_file=1"); total_dls = cursor.fetchone()[0]
        cursor.execute("SELECT AVG(rating) FROM reviews"); avg_rt = cursor.fetchone()[0] or 0
...
async def skip_comment(call: types.CallbackQuery, state: FSMContext):
    ...
    cursor.execute("INSERT OR REPLACE INTO reviews VALUES (?, ?, ?, ?, ?)", (user_id, call.from_user.username or "User", data['rating'], "...", now))
    conn.commit()
...
async def save_comment(message: types.Message, state: FSMContext):
    ...
    data = await state.get_data()
    now = datetime.now(MSK).strftime("%d.%m.%Y %H:%M")
    cursor.execute("INSERT OR REPLACE INTO reviews VALUES (?, ?, ?, ?, ?)", (user_id, message.from_user.username or "User", data['rating'], message.text, now))
    conn.commit()
...
async def get_file_doc(message: types.Message, state: FSMContext):
    ...
    conn.commit()
    await message.answer("✅ Файл сохранен в базу!")
    await state.clear()
