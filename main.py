...
async def admin_file_save(message: types.Message, state: FSMContext):
    ...
    conn.commit()
    await message.answer("✅ Файл успешно сохранен и готов к выдаче!"); await state.clear()
    
@dp.message(AdminStates.waiting_for_file, ~F.document) # Если прислали не документ
async def admin_file_invalid(message: types.Message):
    await message.answer("❌ Это не документ. Пожалуйста, отправь файл в формате документа.")
