...
async def process_download(callback: types.CallbackQuery):
    ...
    if not file_id:
        await callback.answer("❌ файл не был приклиплен к боту", show_alert=True)
        return
    ...
    cursor.execute('''UPDATE users SET received_file=1, last_download_time=?, date_received=? WHERE user_id=?''',
                   (now_str, date_rec, user_id))
    conn.commit()
...
async def cmd_grade(message: types.Message, state: FSMContext):
    ...
    if not row or int(row[0]) == 0:
        await message.answer(f"❌ Бот не видит, что ты получил файл. Сначала нажми 'Скачать драм кит' в /start!{CONTACT_INFO}")
        return
...
async def admin_panel(message: types.Message):
    ...
    text = (f"🛠 <b>АДМИН-ПАНЕЛЬ</b>\n\nФайл: {f_status}\nБот: {b_status}\n\n"
            f"Команды:\n/FileDK, /on, /off, /Stata, /sms, /delete_review")
...
async def admin_stata(message: types.Message):
    if message.from_user.id == ADMIN_ID:
        ...
        await message.answer("📊 Собираю статистику и проверяю подписки, подождите...")
        ...
        # Если текст слишком длинный, Телеграм его не отправит, поэтому режем
        if len(res) > 4096:
            for x in range(0, len(res), 4096):
                await message.answer(res[x:x+4096])
        else:
...
async def admin_del_review(message: types.Message):
    if message.from_user.id == ADMIN_ID:
        ...
        except Exception: await message.answer("Пример: /delete_review 1234567")
