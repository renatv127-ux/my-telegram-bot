...
# --- БАЗА ДАННЫХ (ИСПРАВЛЕНО ДЛЯ СОХРАНЕНИЯ НА RAILWAY) ---
# Получаем путь из переменной окружения Railway. По умолчанию просто bot_data.db
DB_FILE = os.getenv("DB_PATH", "bot_data.db")

# Создаем папку, если путь указывает на /data/ (для Railway Volumes)
...
def db_init():
    ...
    # Проверка на наличие колонки last_msg_id (чтобы не было ошибок на старых базах)
