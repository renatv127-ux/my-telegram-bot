...
async def cmd_grade(message: types.Message, state: FSMContext):
    ...
    # Если пользователя нет в базе или он не получал файл
...
async def process_sub(callback: types.CallbackQuery):
    if await is_subscribed(callback.from_user.id):
        ...
        # ОБНОВЛЕННАЯ ЛОГИКА: Сначала записываем получение в базу
        now = datetime.now(MSK).strftime("%d.%m.%Y %H:%M")
        
        # Проверяем, первое ли это скачивание для этого юзера
        cursor.execute("SELECT received_file FROM users WHERE user_id=?", (callback.from_user.id,))
        user_data = cursor.fetchone()
        
        if not user_data:
            # Если юзера нет (нажал кнопку без /start), создаем его
            cursor.execute("INSERT INTO users (user_id, username, full_name, received_file, date_received) VALUES (?, ?, ?, 1, ?)",
                           (callback.from_user.id, callback.from_user.username, callback.from_user.first_name, now))
            cursor.execute("UPDATE settings SET value = CAST(value AS INTEGER) + 1 WHERE key='downloads'")
        elif user_data[0] == 0:
            # Если юзер был, но еще не качал
            cursor.execute("UPDATE users SET received_file=1, date_received=? WHERE user_id=?", (now, callback.from_user.id))
            cursor.execute("UPDATE settings SET value = CAST(value AS INTEGER) + 1 WHERE key='downloads'")
        
        conn.commit()
        
        # Теперь берем актуальную статистику для сообщения
