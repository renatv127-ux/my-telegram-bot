...
def db_init():
    ...
    cursor.execute("INSERT OR IGNORE INTO settings (key, value) VALUES ('file_id', '')") # Инициализация для файла
    conn.commit()
...
async def admin_file_save(message: types.Message, state: FSMContext):
    ...
    if message.document:
        ...
        conn.commit()
        await message.answer("✅ Файл успешно сохранен и привязан к боту!"); await state.clear()
    else: await message.answer("❌ Ошибка! Нужно отправить именно ФАЙЛ (документ), а не фото.")
...
async def admin_del_review(message: types.Message):
    ...
    try:
        ...
        # Проверяем наличие отзыва именно в таблице reviews
        cursor.execute("SELECT user_id FROM reviews WHERE user_id=?", (rid,))
        if cursor.fetchone():
            cursor.execute("DELETE FROM reviews WHERE user_id=?", (rid,))
            conn.commit()
            await message.answer(f"✅ Отзыв пользователя {rid} удален. Статистика скачиваний не затронута.")
        else:
            await message.answer(f"❌ У пользователя с ID {rid} нет активных отзывов.")
