...
async def process_download(callback: types.CallbackQuery):
    ...
    if u_data and u_data[0]:
        try:
            # Используем формат с секундами для точности
            last_dt = datetime.strptime(u_data[0], "%d.%m.%Y %H:%M:%S").replace(tzinfo=MSK)
            diff = now_dt - last_dt
            if diff < timedelta(minutes=5):
                minutes_left = 5 - (diff.seconds // 60)
                await callback.answer(f"⏳ Подождите еще около {max(1, minutes_left)} мин.", show_alert=True)
                return
    ...
    async with download_queue:
        ...
        # Сохраняем время с секундами
        now_str_full = now_dt.strftime("%d.%m.%Y %H:%M:%S")
        now_str_short = now_dt.strftime("%d.%m.%Y %H:%M") # для даты получения
        
        cursor.execute("""UPDATE users SET 
                          received_file=1, 
                          last_download_time=?, 
                          date_received=COALESCE(date_received, ?) 
                          WHERE user_id=?""", (now_str_full, now_str_short, user_id))
        conn.commit()
