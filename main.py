...
async def cmd_grade(message: types.Message, state: FSMContext):
    # ПРОВЕРКА received_file УДАЛЕНА, ЧТОБЫ ИЗБЕЖАТЬ ОШИБОК
    user_id = message.from_user.id
...
async def admin_del_review(message: types.Message):
    ...
    try:
        ...
        if cursor.fetchone():
            # Если отзыв есть — удаляем его из таблицы reviews
            # Таблица users (которая нужна для /Stata) остается нетронутой
            cursor.execute("DELETE FROM reviews WHERE user_id=?", (rid,))
            conn.commit()
            await message.answer(f"✅ Отзыв пользователя {rid} успешно удален.")
        else:
            # Если отзыва нет — выводим ошибку
