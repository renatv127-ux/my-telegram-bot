...
# --- БАЗА ДАННЫХ (ЖЕСТКАЯ ПРОВЕРКА ПУТИ) ---
DB_FILE = os.getenv("DB_PATH", "/data/bot_data.db")

# Проверяем, существует ли папка для базы
db_dir = os.path.dirname(DB_FILE)
if db_dir and not os.path.exists(db_dir):
    try:
        os.makedirs(db_dir, exist_ok=True)
        print(f"--- ПАПКА {db_dir} СОЗДАНА ---")
    except:
        DB_FILE = "bot_data.db" # Если /data недоступна, сохраняем в корень (временно)
        print("!!! ПРЕДУПРЕЖДЕНИЕ: ДИСК VOLUME НЕ ПОДКЛЮЧЕН. ДАННЫЕ БУДУТ УДАЛЯТЬСЯ !!!")

print(f"--- БОТ ИСПОЛЬЗУЕТ БАЗУ: {DB_FILE} ---")
...
async def process_download(callback: types.CallbackQuery):
    ...
    if not file_id or len(file_id) < 5:
        await callback.answer("❌ Администратор еще не загрузил файл.", show_alert=True)
        return
    ...
    async with download_queue:
        ...
        cursor.execute("""UPDATE users SET received_file=1, last_download_time=?, 
                          date_received=COALESCE(date_received, ?) WHERE user_id=?""", 
                       (now_str, now_str, user_id))
        conn.commit()

        avg, count = get_average_rating()
        total_unique = get_total_downloads()
        
        caption = (f"🥁 <b>Ваш файл готов!</b>\n📈 Скачало человек: {total_unique}\n⭐ Рейтинг: {avg}/5\n\n"
...
async def admin_panel(message: types.Message):
    cur_file = get_setting("file_id")
    f_status = "✅ Загружен" if len(cur_file) > 5 else "❌ Нет файла"
...
async def admin_stata(message: types.Message):
    ...
    rows = cursor.fetchall()
    if not rows:
        await message.answer("Пока никто не скачал.")
        return
...
async def del_review(message: types.Message):
    ...
    except: await message.answer("Формат: /delete_review ID")
