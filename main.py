...
def db_init():
    ...
    # Добавление колонок для старых БД
...
async def set_main_menu(bot: Bot):
    ...
    admin_cmds = user_cmds + [
        ...
        BotCommand(command="clear_stata", description="Сбросить скачивания"),
        BotCommand(command="wipe_users", description="Сброс базы юзеров")
    ]
...
async def process_download(callback: types.CallbackQuery):
    ...
    if await is_banned(user_id):
        await callback.answer("⛔️ Доступ заблокирован!", show_alert=True); return
    ...
    wait_msg = await callback.message.edit_text("⏳ Вы в очереди... Отправка через 4 сек.")
    async with download_queue:
        ...
        caption = f"🥁 <b>Готово!</b>\n📈 Скачало: {len(rows_all_dl() if 'rows_all_dl' in locals() else [])}\n⭐ Рейтинг: {avg}/5\n\n/grade — отзыв"
        try:
...
async def cmd_reviews(message: types.Message):
    await show_reviews(message, 0)

async def show_reviews(message_or_call, page: int):
    limit = 5
    offset = page * limit
    ...
    text = f"⭐ <b>ОТЗЫВЫ ({total})</b>\nСредний рейтинг: {avg}/5\n\n"
    if not rows: text += "Отзывов пока нет."
    else:
        for r in rows: text += f"👤 <b>@{r[0]}</b> | {'⭐'*r[1]}\n📝 {r[2]}\n📅 {r[3]}\n\n"
    
    kb = []
    nav = []
    if page > 0: nav.append(InlineKeyboardButton(text="⬅️", callback_data=f"rev_{page-1}"))
    if total_pages > 1: nav.append(InlineKeyboardButton(text=f"{page+1}/{total_pages}", callback_data="none"))
    if page < total_pages - 1: nav.append(InlineKeyboardButton(text="➡️", callback_data=f"rev_{page+1}"))
    if nav: kb.append(nav)
    
    markup = InlineKeyboardMarkup(inline_keyboard=kb)
    if isinstance(message_or_call, types.Message): await message_or_call.answer(text, reply_markup=markup)
    else: await message_or_call.message.edit_text(text, reply_markup=markup)

@dp.callback_query(F.data.startswith("rev_"))
async def process_rev_page(callback: types.CallbackQuery):
    page = int(callback.data.split("_")[1])
    await show_reviews(callback, page); await callback.answer()
...
async def process_rating(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("✍️ Напишите Ваш отзыв (макс. 200 символов):")
    await state.set_state(ReviewStates.waiting_for_comment)

@dp.message(ReviewStates.waiting_for_comment)
async def save_comment(message: types.Message, state: FSMContext):
    if len(message.text) > 200:
        await message.answer("❌ Отзыв слишком длинный! Напишите до 200 символов."); return
...
async def full_stats_cmd(message: types.Message):
    ...
    cursor.execute("SELECT COUNT(*) FROM users WHERE received_file=1"); total_dl = cursor.fetchone()[0]
    ...
    res += f"📥 <b>Уникальных скачиваний:</b> {total_dl}\n\n📋 <b>Последние 15 скачавших:</b>\n"
...
async def stata_list(message: types.Message):
    cursor.execute("SELECT user_id, username, date_received FROM users WHERE received_file=1")
    rows = cursor.fetchall()
    if not rows: await message.answer("Нет скачиваний."); return
...
async def ban_user(message: types.Message):
    try:
        ...
        conn.commit(); await message.answer(f"🚫 ID {uid} забанен.")
    except: await message.answer("Юзай: /ban ID")
...
async def unban_user(message: types.Message):
    try:
        ...
        conn.commit(); await message.answer(f"✅ ID {uid} разбанен.")
    except: await message.answer("Юзай: /unban ID")
...
async def banlist_cmd(message: types.Message):
    ...
    rows = cursor.fetchall()
    if not rows: await message.answer("Банов нет."); return
    res = "🚫 <b>БАН-ЛИСТ:</b>\n\n"
    for r in rows: res += f"• <code>{r[0]}</code> (@{r[1]})\n"
    await message.answer(res)

@dp.message(Command("wipe_users"), F.from_user.id == ADMIN_ID)
async def wipe_users(message: types.Message):
    cursor.execute("DELETE FROM users"); conn.commit(); await message.answer("⚠️ <b>БАЗА ЮЗЕРОВ ПОЛНОСТЬЮ ОЧИЩЕНА!</b>")
...
async def del_rev(message: types.Message):
    try:
        ...
        conn.commit(); await message.answer(f"🗑 Отзыв юзера {uid} удален.")
    except: await message.answer("Юзай: /delete_review ID")
...
async def clear_r(message: types.Message):
...
async def clear_s(message: types.Message):
    cursor.execute("UPDATE users SET received_file=0, last_download_time=0, date_received=NULL"); conn.commit(); await message.answer("✅ Скачивания сброшены.")
...
async def process_file_up(message: types.Message, state: FSMContext):
...
async def cmd_help(message: types.Message):
    ...
    text = "📖 <b>Команды:</b>\n/start - Получить файл\n/grade - Оценить\n/review - Отзывы\n/help - Помощь"
    if is_admin: text += "\n\n👑 <b>Админ:</b> /admin, /full_stats, /Stata, /banlist, /sms, /wipe_users"
