...
async def cmd_start(message: types.Message, state: FSMContext):
    ...
    # Запись пользователя с датой захода
...
async def rate_sel(call: types.CallbackQuery, state: FSMContext):
    ...
    await call.message.edit_text("✍️ Напишите краткий отзыв (<b>до 200 символов</b>):")
...
async def cmd_rev_list(message: types.Message):
    ...
    if not rows: await message.answer("Отзывов нет."); return
    res = "💬 <b>Последние отзывы:</b>\n\n"
    for r in rows: res += f"👤 @{r[0]} | {'⭐'*r[1]}\n📝 {r[2]}\n📅 {r[3]}\n\n"
    await message.answer(res)

# --- АДМИНКА (СТАТИСТИКА ПО ШАБЛОНУ) ---
...
async def cmd_help(message: types.Message):
    ...
    if message.from_user.id == ADMIN_ID:
        txt += "\n\n👑 <b>Админ:</b> /admin, /full_stats, /set_file, /wipe_users"
