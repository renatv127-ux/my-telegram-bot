...
# --- ПУТЬ К БАЗЕ ДАННЫХ (ФИКС ДЛЯ RAILWAY) ---
# Если в Railway подключен Volume с путем /data, база будет жить там вечно.
if os.path.exists("/data"):
    DB_PATH = "/data/bot_data.db"
else:
    # Локальный путь для тестов на ПК
...
def get_average_rating():
    res = db_query("SELECT AVG(rating), COUNT(user_id) FROM reviews", fetchone=True)
...
async def start_cmd(message: types.Message):
    ...
    db_query("UPDATE users SET last_msg_id=? WHERE user_id=?", (sent_msg.message_id, user_id), commit=True)
...
async def process_download(callback: types.CallbackQuery):
    ...
    if not file_id or len(file_id) < 5:
        await callback.answer("❌ Файл еще не был загружен администратором!", show_alert=True)
        return

    u_data = db_query("SELECT last_download_time, received_file, date_received FROM users WHERE user_id=?", (user_id,), fetchone=True)
    ...
    db_query("UPDATE users SET received_file=1, last_download_time=?, date_received=? WHERE user_id=?", (now_str, date_rec, user_id), commit=True)
...
async def cmd_grade(message: types.Message, state: FSMContext):
    existing = db_query("SELECT rating, comment, date FROM reviews WHERE user_id=?", (message.from_user.id,), fetchone=True)
    if existing:
        try:
...
async def save_comment(message: types.Message, state: FSMContext):
    ...
    row = db_query("SELECT date FROM reviews WHERE user_id=?", (message.from_user.id,), fetchone=True)
    ...
    db_query("INSERT OR REPLACE INTO reviews (user_id, username, rating, comment, date) VALUES (?, ?, ?, ?, ?)",
             (message.from_user.id, message.from_user.username or message.from_user.first_name, data['rating'], message.text, dt), commit=True)
...
async def skip_comment(callback: types.CallbackQuery, state: FSMContext):
    data = await state.get_data()
    row = db_query("SELECT date FROM reviews WHERE user_id=?", (callback.from_user.id,), fetchone=True)
    dt = row[0] if row else datetime.now(MSK).strftime("%d.%m.%Y %H:%M")
    db_query("INSERT OR REPLACE INTO reviews (user_id, username, rating, comment, date) VALUES (?, ?, ?, ?, ?)",
             (callback.from_user.id, callback.from_user.username or callback.from_user.first_name, data['rating'], "Без описания", dt), commit=True)
...
async def view_reviews(message: types.Message):
    avg, count = get_average_rating()
    rows = db_query("SELECT username, rating, comment, date, user_id FROM reviews ORDER BY date DESC LIMIT 10", fetchall=True)
    res = f"⭐ <b>Средний рейтинг: {avg}/5</b> (Отзывов: {count})\n\n"
    if rows:
        for r in rows:
            id_info = f"🆔 ID: <code>{r[4]}</code>\n" if message.from_user.id == ADMIN_ID else ""
            res += f"👤 @{r[0]} | {r[1]}/5\n{id_info}📝 {r[2]}\n📅 {r[3]}\n\n"
    else: res = "Отзывов пока нет. "
...
async def admin_file_save(message: types.Message, state: FSMContext):
    ...
    db_query("UPDATE settings SET value=? WHERE key='file_id'", (fid,), commit=True)
    await message.answer(f"✅ Файл сохранен в базу!\nID: <code>{fid}</code>")
    await state.clear()
...
async def admin_stata(message: types.Message):
    if message.from_user.id != ADMIN_ID: return
    rows = db_query("SELECT user_id, username, date_received FROM users WHERE received_file=1", fetchall=True)
    ...
    for r in rows:
        ...
        res += f"ID: <code>{r[0]}</code> | @{r[1]} | {r[2]} | Sub: {sub_status}\n"
...
async def admin_del_review(message: types.Message):
    ...
    try:
        ...
        db_query("DELETE FROM reviews WHERE user_id=?", (rid,), commit=True)
        await message.answer(f"✅ Если отзыв {rid} был, он удален.")
    except: await message.answer("Пример: /delete_review 1234567")

async def main(): 
    db_init()
    print("--- БОТ ЗАПУЩЕН (RAILWAY MODE) ---")
