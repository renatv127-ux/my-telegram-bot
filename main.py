...
import asyncio
import sqlite3
import time # Используем для максимально точного времени
...
def db_init():
    cursor.execute('''CREATE TABLE IF NOT EXISTS users 
                      (user_id INTEGER PRIMARY KEY, username TEXT, full_name TEXT, 
                       ...
                       last_download_time REAL DEFAULT 0)''') # REAL для хранения чисел
...
async def process_download(callback: types.CallbackQuery):
    ...
    # --- НОВАЯ СВЕРХТОЧНАЯ ПРОВЕРКА КУЛДАУНА ---
    ...
    if u_data and u_data[0] > 0:
        ...
        if time_passed < 300: # 300 секунд = 5 минут
    ...
    async with download_queue:
        ...
        # Обновляем данные: время в секундах и дату текстом для статистики
